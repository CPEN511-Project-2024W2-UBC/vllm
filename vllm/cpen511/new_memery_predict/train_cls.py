...
mod = 100
epoch = 30
data_file_path = os.path.dirname(os.path.abspath(
    __file__)) + "/../data/pure_sequence.csv"
...
# Initialize model
model = MlpMemoryPredictClassification(
    window_size=window_size, num_classes=mod).cuda()
...
with torch.no_grad():
    for inputs, label, input_onehot, label_onehot in test_loader:
        ...
        print(
            f"Output: {outputs.tolist()}, Label: {(label%mod).tolist()}, correct: {correct}, total: {total}")
print(
    f"Test Loss: {test_loss / len(test_loader)}, Accuracy: {(correct / total) * 100}%")
